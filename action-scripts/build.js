...
const parseLineHeight = (value) => typeof value === "string" ? value : `${value}px`;
...
const formatTwTypographyValues = (obj) => {
    ...
    items.forEach(([k, v]) => {
        if (v[k])
            return (expanded["." + k] = v[k]);
        if (k === "ios")
            return (expanded[".ios"] = v);
...
StyleDictionary.registerTransform({
    ...
    transformer: ({ value }) => {
        ...
        const finalValues = values
            .map(({ color, type, x, y, blur, spread }) => `${type === "innerShadow" ? "inset " : ""}${x}px ${y}px ${blur}px ${spread}px ${color}`)
            .join(", ");
        return finalValues;
    },
});
...
StyleDictionary.registerFormat({
    ...
    formatter: ({ dictionary = {} }) => {
        ...
        const entries = {};
        const keys = Object.keys(tokens)?.filter((k = "") => k[0] !== "_" && !excluded.includes(k));
        keys.forEach((key) => {
            const singularized = key === "fontFamilies"
                ? "fontFamily"
                : key === "lineHeights"
                    ? "lineHeight"
                    : key === "fontWeights"
