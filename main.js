"use strict";
const { buildConfig } = require("./action-scripts/build-config.ts");
const { build } = require("./action-scripts/build.ts");
const { copyThemeFile } = require("./action-scripts/copy-theme.ts");
const { exec } = require("child_process");
const shipwrightScript = ({ figmaTokenFile = "token/token.json", outputFolder = "build", styleSystem, actionPath = "./", }) => {
...
// script execution
const [figmaTokenFile, outputFolder, styleSystem, actionPath] = process.argv.slice(2);
